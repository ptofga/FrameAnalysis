...
#  statistic cost by render pass
#init render pass name	
	renderpassname= ""
	#init accumulate list 
	accumulate_list = [0,0,0]
	dict_list = []
	dict = {}
	for( index, row) in comparison_df.iterrows() : 
		if renderpassname == "" :
			renderpassname = row.Symbol
			for i in range(numberofGPU):
				accumulate_list[i]= int(row[platform_list[i]])
		else :
			if renderpassname == row.Symbol : 
				for i in range(numberofGPU):
					accumulate_list[i] += int(row[platform_list[i]])
			else :
				#str_temp = renderpassname+": "+str(accumulate)
				#print(str_temp)
				dict = {}
				dict['Symbol']=renderpassname
				#print (renderpassname)
				for i in range(numberofGPU):
					dict[platform_list[i]]=accumulate_list[i]
					#print(str(accumulate_list[i]))
				#print(dict)
	...
	for i in range(numberofGPU):
		dict[platform_list[i]]=accumulate_list[i]
	dict_list.append(dict)
	renderpass_df= pandas.DataFrame(dict_list)
	renderpass_df.to_csv("renderpass.csv")
...
for s in info_df.Platform:
	title_str += s + ":"+str(int(1000*1000/comparison_df[s].values.sum()))+"FPS "
plt.title(title_str)
...
def on_add(sel):
	...
	if symbol_list.__len__() > 0:
		sel.annotation.set_text(
			'Draw call No.{}\n{}\n{}'.format(int(sel.index), symbol_list[int(sel.index)], comment_str))
	else:
		sel.annotation.set_text(
			'Draw call No.{}\n{}'.format(int(sel.index), comment_str))
